        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                <div class="flex gap-4 mt-2">
                    <div>
                        <p class="text-3xl font-bold text-slate-800">${eqpsDisponiveis}</p>
                        <p class="text-xs text-slate-500 font-medium uppercase tracking-wider mt-1">Disponíveis</p>
                    </div>
                    <div class="border-l border-slate-200 pl-4">
                        <p class="text-3xl font-bold text-slate-800">${eqpsEmUso}</p>
                        <p class="text-xs text-slate-500 font-medium uppercase tracking-wider mt-1">Em Uso</p>
                    </div>
                </div>
...
const renderEquipamentos = (container, headerActions) => {
    headerActions.innerHTML = `
        <div class="flex flex-wrap items-center gap-4">
            <div class="relative">
                ...
                <input type="text" id="search-equip" placeholder="Buscar equipamento..." class="pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none w-48 lg:w-64">
            </div>
            <select id="filter-status-equip" class="py-2 px-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                <option value="ALL">Todos os Status</option>
                <option value="DISPONIVEL">Disponíveis</option>
                <option value="EM_USO">Em Uso</option>
            </select>
            <select id="sort-equip" class="py-2 px-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
    ...
    const renderTable = () => {
        ...
        if (equipamentos.length === 0) {
            tableHTML += `<tr><td colspan="4" class="py-6 text-center text-slate-500">Nenhum equipamento encontrado.</td></tr>`;
        } else {
            equipamentos.forEach(eqp => {
                ...
                if (isDisponivel) {
                    alocarBtn = `
                        <button data-id="${eqp.id}" class="btn-alocar-manual bg-slate-100 text-slate-600 hover:bg-primary hover:text-white px-3 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-1 mr-2" title="Alocar Manualmente para funcionário (sem gerar termo)">
        ...
        document.querySelectorAll('.btn-alocar-manual').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const formHTML = `
                    <div class="p-6">
                        <h3 class="text-xl font-bold text-slate-800 mb-2">Alocação Manual</h3>
                        <p class="text-sm text-slate-500 mb-6">Usado para registrar equipamentos que já foram entregues anteriormente sem passar pela geração do Termo via sistema.</p>
                        ...
                        <form id="form-alocar" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Quem está com este equipamento?</label>
                                <input type="text" id="search-alocar-func" placeholder="Buscar funcionário por nome..." class="w-full border border-slate-300 rounded-t-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none mb-0 border-b-0">
                                <select name="funcionarioId" id="select-alocar-func" required size="5" class="w-full border border-slate-300 rounded-b-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Data da Entrega</label>
                                <input type="date" name="dataEntrega" required max="${todayStr}" value="${todayStr}" class="w-full border border-slate-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                            </div>
                            <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-slate-100">
                                <button type="button" class="btn-cancel bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg font-medium hover:bg-slate-50 transition-colors">Cancelar</button>
...
const renderFuncionarios = (container, headerActions) => {
    headerActions.innerHTML = `
        <div class="flex flex-wrap items-center gap-4">
            <div class="relative">
                <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2"></i>
                <input type="text" id="search-func" placeholder="Buscar funcionário..." class="pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none w-64 lg:w-80">
            </div>
...
window.renderFuncionarioPerfil = (id) => {
    ...
    const historicoHTML = historico.length === 0 ?
        '<p class="text-sm text-slate-500 py-4 text-center">Nenhum registro no histórico.</p>' :
        historico.map(h => {
            ...
            if (h.tipo === 'DEVOLUCAO_COMPLETA' && h.equipamentosIds) {
                ...
                eqpContent = `<p class="text-sm font-medium text-slate-800">${acao} múltiplos itens:</p>
                              <div class="mt-1 space-y-1 pl-2 border-l-2 border-slate-200">
                                ${eqps.map(eq => `<div><span class="font-bold text-slate-900">${eq.descricao}</span> <span class="text-xs text-slate-500">(${eq.modeloMarca})</span></div>`).join('')}
                              </div>`;
            } else {
                const eqp = equipamentos.find(e => e.id === h.equipamentoId) || { descricao: 'Equipamento Excluído', modeloMarca: '' };
                eqpContent = `<p class="text-sm font-medium text-slate-800">${acao} <span class="font-bold text-slate-900">${eqp.descricao}</span></p>
                              <p class="text-xs text-slate-500 mt-1">${eqp.modeloMarca}</p>`;
            }
    ...
    const posseHTML = equipamentosEmPosse.length === 0 ?
        '<p class="text-sm text-slate-500 py-4 text-center border border-dashed border-slate-300 rounded-lg">Nenhum equipamento em posse atualmente.</p>' :
        `<div class="space-y-3">
             ${equipamentosEmPosse.map(eqp => `
                <div class="flex items-center justify-between p-3 border border-slate-200 rounded-lg bg-white">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-bold text-slate-800">${eqp.descricao}</p>
                            <p class="text-xs text-slate-500">${eqp.modeloMarca}</p>
                        </div>
                    </div>
                    <button data-eqpid="${eqp.id}" class="btn-devolver bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 hover:text-red-600 px-3 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-1" title="Registrar devolução do equipamento">
    ...
    const html = `
        <div class="flex flex-col h-[85vh] max-h-[800px]">
            <!-- Header Fixa -->
            <div class="p-6 border-b border-slate-200 bg-white shrink-0 flex justify-between items-start">
                <div>
                    <h3 class="text-2xl font-bold text-slate-800">${funcionario.nome}</h3>
                    <p class="text-slate-500">${funcionario.funcao} &bull; Admissão: ${formatInputDate(funcionario.dataAdmissao)}</p>
                </div>
                <button onclick="hideModal()" class="text-slate-400 hover:text-slate-600 p-1">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2">
                                ...
                                <h4 class="text-lg font-bold text-slate-800">Posse Atual</h4>
                            </div>
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-4 pb-2 border-b border-slate-200">
                            ...
                            <h4 class="text-lg font-bold text-slate-800">Linha do Tempo</h4>
                        </div>
                        <div class="pl-2">
                            ${historicoHTML}
                        </div>
...
const renderHistoricoGeral = (container, headerActions, filters = {}) => {
    headerActions.innerHTML = `
        <div class="flex flex-col sm:flex-row items-center gap-3">
            <div class="flex items-center gap-2 text-sm text-slate-600 bg-white border border-slate-200 rounded-lg px-3 shadow-sm">
                <i data-lucide="filter" class="w-4 h-4 text-slate-400"></i>
                <input type="date" id="filter-hist-start" value="${filters.start || ''}" class="border-none bg-transparent py-2 focus:ring-0 text-slate-700 outline-none w-[120px] text-xs sm:text-sm cursor-pointer">
                <span class="text-slate-300">até</span>
                <input type="date" id="filter-hist-end" value="${filters.end || ''}" class="border-none bg-transparent py-2 focus:ring-0 text-slate-700 outline-none w-[120px] text-xs sm:text-sm cursor-pointer">
            </div>
            <div class="flex items-center gap-2">
                <button id="btn-print-historico" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg font-medium transition-colors shadow-sm inline-flex items-center gap-2">
    ...
    if (agrupadoHistorico.length === 0) {
        ...
        agrupadoHistorico.forEach(h => {
            ...
            if ((h.tipo === 'DEVOLUCAO_COMPLETA' || h.tipo === 'ENTREGA_AGRUPADA') && h.equipamentosIds) {
                ...
                const eqp = equipamentos.find(e => e.id === h.equipamentoId) || { descricao: 'Equip. Excluído', modeloMarca: '' };
                eqpContent = `<span class="block text-slate-800 font-medium">${eqp.descricao}</span><span class="block text-xs text-slate-500">${eqp.modeloMarca}</span>`;
            }

            tableHTML += `
                <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td class="py-4 px-6 text-slate-600 whitespace-nowrap">
                        <span class="font-medium text-slate-800">${dataStr}</span>
                        <span class="text-xs text-slate-400 block">${timeStr}</span>
                    </td>
                    <td class="py-4 px-6">${badge}</td>
                    <td class="py-4 px-6 font-medium text-slate-800">${fnc.nome}</td>
                    <td class="py-4 px-6">
                        ${eqpContent}
                    </td>
                </tr>
    ...
    tableHTML = `
        <div class="mb-6">
            <h2 class="text-xl font-bold text-slate-800">Histórico de Movimentações</h2>
            <p class="text-slate-500 text-sm mt-1">Acompanhamento completo de entregas e devoluções de equipamentos na empresa.</p>
        </div>
        ${tableHTML}
    `;
